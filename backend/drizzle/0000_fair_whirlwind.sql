CREATE TABLE `seat_allocations` (
	...
	`hall` enum('ZIA_HALL','SELIM_HALL','HAMID_HALL','SHAHIDUL_HALL','TIN_SHED_HALL','FAZLUL_HUQ_HALL') NOT NULL,
...
CREATE TABLE `seat_applications` (
	...
	`student_id` varchar(36) NOT NULL,
	`roll_number` varchar(20) NOT NULL,
	`hall` enum('ZIA_HALL','SELIM_HALL','HAMID_HALL','SHAHIDUL_HALL','TIN_SHED_HALL','FAZLUL_HUQ_HALL'),
	`academic_department` enum('CSE','EEE','ME','CE','IPE','ECE','ETE','BME','MTE','URP','ChE','Arch') NOT NULL,
	`session` varchar(10) NOT NULL,
	`seat_application_status` enum('PENDING','APPROVED','REJECTED') NOT NULL DEFAULT 'PENDING',
...
CREATE TABLE `hall_admins` (
	...
	`academic_department` enum('CSE','EEE','ME','CE','IPE','ECE','ETE','BME','MTE','URP','ChE','Arch'),
	`hall` enum('ZIA_HALL','SELIM_HALL','HAMID_HALL','SHAHIDUL_HALL','TIN_SHED_HALL','FAZLUL_HUQ_HALL') NOT NULL,
	...
	`hall_admin_status` enum('PENDING','APPROVED','REJECTED') NOT NULL DEFAULT 'PENDING',
...
CREATE TABLE `damage_reports` (
	...
	`damage_cost` int unsigned,
	`is_student_responsible` boolean,
	`manager_note` text,
	`liable_student_id` varchar(36),
	`damage_report_status` enum('REPORTED','VERIFIED','FIXED') NOT NULL DEFAULT 'REPORTED',
...
ALTER TABLE `damage_reports` ADD CONSTRAINT `damage_reports_liable_student_id_uni_students_id_fk` FOREIGN KEY (`liable_student_id`) REFERENCES `uni_students`(`id`) ON DELETE set null ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE `damage_reports` ADD CONSTRAINT `damage_reports_fixed_by_hall_admins_id_fk` FOREIGN KEY (`fixed_by`) REFERENCES `hall_admins`(`id`) ON DELETE set null ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX `idx_uni_students_academic_department` ON `uni_students` (`academic_department`);--> statement-breakpoint
...
CREATE INDEX `idx_damage_hall` ON `damage_reports` (`hall`);--> statement-breakpoint
CREATE INDEX `idx_damage_status` ON `damage_reports` (`damage_report_status`);--> statement-breakpoint
CREATE INDEX `idx_damage_liable_student` ON `damage_reports` (`liable_student_id`);