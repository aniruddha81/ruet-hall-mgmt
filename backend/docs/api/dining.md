...
| `POST` | `/book-tokens` | `multipart` `receiptImage` for `BANK`. `BKASH`/`NAGAD`/`ROCKET` return `{ gatewayUrl, status: PENDING }` for SSLCommerz. Optional JSON `returnUrl` for mobile deep-link return |
